...
class NewChemformer(Chemformer):
    ...
    def build_model(self, args):
        ...
        model = self.model
        old_encoder_forward = model.encoder.forward
        ...
        # Patch encoder: apply FiLM after encoding
        def new_encoder_forward(*args, **kwargs):
            memory = old_encoder_forward(*args, **kwargs)
            if hasattr(model, '_geom') and model._geom is not None:
                device = memory.device
                geom = model._geom.float().to(device)
        ...
        model.encoder.forward = new_encoder_forward

        # Minimal forward patch: only extract geom from batch
        def new_forward(batch, *a, **kw):
            model._geom = batch.get("geom")
            out = old_forward(batch, *a, **kw)
            model._geom = None
            return out
