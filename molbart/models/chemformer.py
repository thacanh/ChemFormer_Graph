...
class Chemformer:
    ...
    def set_datamodule(
        ...
        # On Windows, datamodule already sets num_workers=0 to avoid pickling issues
        # Don't override it unless we actually have GPUs
        import platform
        if platform.system() == 'Windows':
            # Keep the Windows-safe num_workers=0 from base.py
            n_workers = 0
        else:
            # Windows-compatible: os.sched_getaffinity không có trên Windows
            try:
